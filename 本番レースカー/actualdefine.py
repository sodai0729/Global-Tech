...
def bothwallfollow1(scan,A):
    right_angle = [i for i in range(35+A,41+A,5)]
    left_angle = [i for i in range(320+A,336+A,5) ]
...
def bothwallfollow2(scan,A):
    right_angle = [i for i in range(50+A,81+A,5)]#もとは７１
    left_angle = [i for i in range(280+A,311+A,5) ]
...
def Speedcontrol(scan,angle,A):
    ...
    #if right_distance < 500 and left_distance < 500:
        #MAX_speed = 0.1
    if right_distance < 400 and left_distance <400:
        MAX_speed = 0.1
    else:
        MAX_speed = 1.0
...
def switch_wall_mode2(scan,A,mode):
    ...
    for i in range(len(switch_right_angle_to_stright)):
        ab.append(rc_utils.get_lidar_average_distance(scan, a[i], 6))
